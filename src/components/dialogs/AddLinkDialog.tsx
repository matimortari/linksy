...
export default function AddLinkDialog({ isOpen, onClose, onSubmit }) {
	...
	const handleFormSubmit = (data: LinkFormData) => {
		onSubmit({ title: data.title, url: data.url })
	}
		...
		<Dialog isOpen={isOpen} onClose={onClose} title="Add New Link">
			<form onSubmit={handleSubmit(handleFormSubmit)} className="my-4 flex flex-col gap-2">
				...
				<div className="input-group">
					<button type="submit" title="Add Link" className="btn bg-success">
						Add Link
					</button>
